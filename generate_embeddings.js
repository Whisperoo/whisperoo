...
async function generateAllEmbeddings() {
  try {
    ...
    // Verify the results
    const { data: embeddings, error: queryError } = await supabase.from(
      "expert_embeddings",
    ).select(`
        expert_id,
    ...
    embeddings.forEach((embedding) => {
      console.log(
        `- ${embedding.profiles.first_name}: ${embedding.profiles.expert_specialties?.join(", ") || "No specialties"}`,
      );
    });
  } catch (error) {
