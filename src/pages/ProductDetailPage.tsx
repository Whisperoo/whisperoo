...
import { Star, Eye, Video, FileText, Clock, FileIcon, ArrowLeft, Heart, Share, Play, Users } from 'lucide-react';
...
export const ProductDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    if (user && productId) {
      const checkPurchase = async () => {
        const purchased = await productService.hasUserPurchased(user.id, productId);
  ...
  useEffect(() => {
    const loadProductFiles = async () => {
      if (product && (product.has_multiple_files || product.total_files_count > 1)) {
        try {
  ...
  // Course helpers
  const isCourse = productFiles.length > 1 && productFiles.some(file => file.file_type === 'video' || file.mime_type?.startsWith('video/'));
  const getTotalDuration = () => {
    if (!isCourse) return product?.duration_minutes;
    return productFiles.reduce((total, file) => total + (file.duration_minutes || 0), 0);
  };
  const hasContent = product?.file_url || product?.primary_file_url || productFiles.length > 0;
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardContent className="p-0">
                <div className="relative aspect-video bg-gradient-to-br from-slate-50 to-slate-100 rounded-lg overflow-hidden">
                  {product.thumbnail_url && product.thumbnail_url.trim() && !product.thumbnail_url.includes('placeholder') ? <div className="relative">
                      <img src={product.thumbnail_url} alt={product.title} className="w-full h-full object-cover" onError={e => {
                    ...
                    if (fallback) fallback.style.display = 'flex';
                      ...
                      <div className="absolute inset-0 w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100" style={{
                    display: 'none'
                  }}>
                        <div className="flex flex-col items-center">
                          <div className={`w-20 h-20 rounded-2xl flex items-center justify-center shadow-lg ${product.product_type === 'video' ? 'bg-gradient-to-br from-red-500 to-red-600' : product.product_type === 'consultation' ? 'bg-gradient-to-br from-green-500 to-green-600' : 'bg-gradient-to-br from-blue-500 to-blue-600'}`}>
                            {product.product_type === 'video' ? <Play className="h-10 w-10 text-white ml-1" /> : product.product_type === 'consultation' ? <Users className="h-10 w-10 text-white" /> : <FileText className="h-10 w-10 text-white" />}
                          </div>
                      ...
                      {(product.product_type === 'video' || isCourse) && hasContent && <div className="absolute inset-0 flex items-center justify-center bg-black/20 backdrop-blur-sm hover:bg-black/30 transition-colors">
                          <Button size="lg" className="rounded-full h-16 w-16 bg-white/90 hover:bg-white text-gray-900 shadow-lg" onClick={handleViewContent}>
                            <Play className="h-6 w-6 ml-1" />
                          </Button>
                        </div>}
                    </div> : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100">
                      <div className="flex flex-col items-center">
                        <div className={`w-20 h-20 rounded-2xl flex items-center justify-center shadow-lg ${product.product_type === 'video' ? 'bg-gradient-to-br from-red-500 to-red-600' : product.product_type === 'consultation' ? 'bg-gradient-to-br from-green-500 to-green-600' : 'bg-gradient-to-br from-blue-500 to-blue-600'}`}>
                          {product.product_type === 'video' ? <Play className="h-10 w-10 text-white ml-1" /> : product.product_type === 'consultation' ? <Users className="h-10 w-10 text-white" /> : <FileText className="h-10 w-10 text-white" />}
                        </div>
                  ...
                  <Badge className="absolute top-4 left-4 capitalize bg-white/90 text-gray-800 hover:bg-white">
                    {isCourse ? 'Course' : product.product_type === 'consultation' ? 'Consultation' : product.product_type}
                  </Badge>
                  
                  {/* Lessons Count for Courses */}
                  {isCourse && <Badge className="absolute top-4 right-4 bg-blue-600 hover:bg-blue-700">
                      {productFiles.length || product.total_files_count} lessons
                    </Badge>}
                  
                  {/* Preview Button for non-video content */}
                  {product.product_type !== 'video' && !isCourse && hasContent && <div className="absolute bottom-4 right-4">
                      <Button size="sm" variant="outline" className="bg-white/90 hover:bg-white backdrop-blur-sm" onClick={handleViewContent}>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{isCourse ? 'About This Course' : 'About This Product'}</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardContent className="p-6">
                ...
                {/* Expert Info */}
                {product.expert && <div className="flex items-center gap-3 mb-4">
                    <Avatar>
                ...
                {/* Rating */}
                {product.average_rating !== undefined && product.average_rating !== null && <div className="flex items-center gap-2 mb-4">
                ...
                {/* Categories */}
                {product.categories && product.categories.length > 0 && <div className="flex flex-wrap gap-2 mb-4">
                    {product.categories.map(cat => <Badge key={(cat as any).category?.id || (cat as any).id} variant="outline">
          {(cat as any).category?.name || (cat as any).name}
                ...
                <div className="space-y-4">
                  {isPurchased && purchaseInfo ? <div className="space-y-2">
                      <div className="text-lg text-muted-foreground">You paid</div>
