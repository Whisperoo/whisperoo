...
export const ProductEditModal: React.FC<ProductEditModalProps> = ({
  ...
  const [fileManagementMode, setFileManagementMode] = useState<'replace' | 'add' | 'manage'>('manage');
  ...
  const existingCategoryIds = product.categories?.map(c => 
    typeof c === 'object' && 'category' in c ? (c.category as any)?.id : (c as any).id
  ).filter(Boolean) || [];

  // Determine content type based on files
  const determineContentType = (filesCount: number): 'single' | 'bundle' | 'course' | 'collection' => {
    if (filesCount <= 1) return 'single';
    return (product.content_type as 'single' | 'bundle' | 'course' | 'collection') || 'bundle';
  };
  ...
  const form = useForm<ProductFormData>({
    ...
    defaultValues: {
      ...
      contentType: determineContentType((product.files?.length || 0) + (product.file_url ? 1 : 0)),
  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      const filesCount = (product.files?.length || 0) + (product.file_url ? 1 : 0);
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      ...
      const updateData: any = {
        ...
        has_multiple_files: (existingFiles.length + newFiles.length - filesToDelete.length) > 1,
        total_files_count: existingFiles.length + newFiles.length - filesToDelete.length,
      };
      ...
      if (newFiles.length > 0) {
        ...
        const hasPrimaryFile = existingFiles.some(f => f.is_primary && !filesToDelete.includes(f.id));
      ...
      const primaryFile = existingFiles.find(f => f.is_primary && !filesToDelete.includes(f.id));
      if (primaryFile) {
        ...
        <div className="sticky top-0 z-10 px-4 py-3 border-b bg-white/95 backdrop-blur-sm">
          <div className="flex items-start justify-between">
            <div className="flex-1 min-w-0 pr-4">
              <DialogTitle className="text-lg font-semibold text-gray-900">Edit Product</DialogTitle>
