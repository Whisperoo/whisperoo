...
export const ExpertDashboard: React.FC = () => {
  ...
  const { data: products, isLoading: productsLoading, refetch: refetchProducts } = useQuery({
    ...
    queryFn: () => profile ? productService.getExpertProducts(profile.id) : [],
    enabled: !!profile,
  });
  ...
  const { data: analytics, isLoading: analyticsLoading } = useQuery({
    ...
    queryFn: () => profile ? productService.getExpertSalesAnalytics(profile.id) : null,
    enabled: !!profile,
  });
    ...
    <div className="container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          ...
          <Button onClick={() => setUploadModalOpen(true)} className="gap-2 w-full sm:w-auto" size="sm">
                        ...
                        <TableCell>
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              {(product.files?.length || (product.file_url ? 1 : 0)) > 0 && (
                                <div className="text-xs text-muted-foreground mt-1">
                                  {product.files?.length || 1} file{(product.files?.length || 1) > 1 ? 's' : ''}
                                </div>
                              )}
                            </div>
                          </div>
            ...
            <CardContent>
              {consultationsLoading ? (
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="min-w-[120px]">Consultation</TableHead>
                        <TableHead className="min-w-[80px]">Amount</TableHead>
                        <TableHead className="min-w-[100px]">Booked Date</TableHead>
                  ...
                  <TableBody>
                    {consultations.map((consultation) => (
                      <TableRow key={consultation.id}>
                        <TableCell>
                          <div className="flex items-center gap-3">
                            {(consultation as any).user?.profile_image_url && (
                              <img
                                src={(consultation as any).user.profile_image_url}
                                alt={(consultation as any).user.first_name || 'Client'}
                                className="w-8 h-8 object-cover rounded-full"
                              />
                            )}
                            <div>
                              <div className="font-medium">
                                {(consultation as any).user?.first_name || 'Client'}
                              </div>
                        ...
                        <TableCell>{formatCurrency(Number(consultation.amount))}</TableCell>
                        <TableCell>
                          {new Date(consultation.purchased_at).toLocaleDateString()}
                        </TableCell>
                        <TableCell>
                          <Badge
                          ...
                          {consultation.consultation_completed_at && (
                            <div className="text-xs text-muted-foreground mt-1">
                              Completed: {new Date(consultation.consultation_completed_at).toLocaleDateString()}
                            </div>
                          )}
                        </TableCell>
                        <TableCell className="text-right">
                          {!consultation.consultation_completed && (
                            <Button
                              size="sm"
                              onClick={() => markConsultationComplete.mutate(consultation.id)}
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-2 text-sm font-semibold">No consultation bookings</h3>
                  <p className="mt-1 text-sm text-muted-foreground">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">Average Product Rating</span>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{profile.expert_total_reviews || 0}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">Account Status</span>
                    <Badge variant={profile.expert_verified ? 'default' : 'secondary'}>
                      {profile.expert_verified ? 'Verified' : 'Pending'}
                    </Badge>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">Consultation Rate</span>
                    <span className="font-medium">
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              the product and all associated files. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
