...
type ProductCategoryMapping =
  Database["public"]["Tables"]["product_category_mappings"]["Row"];
type ProductFile = Database["public"]["Tables"]["product_files"]["Row"];
type ProductFileInsert =
  Database["public"]["Tables"]["product_files"]["Insert"];
...
export const productService = {
  ...
  // ... (all other methods remain exactly as you have them) ...
  ...
  async uploadProductFile(
    ...
    const filePath = STORAGE_PATHS.productFile(
      expertId,
      productId,
      fileId,
      fileExt,
  ...
  async uploadProductThumbnail(
    ...
    const filePath = STORAGE_PATHS.productThumbnail(
      expertId,
      productId,
      fileExt,
    );
  ...
  async addProductFile(
    ...
    console.log(
      `Uploading file: ${file.name} (${(file.size / 1024 / 1024).toFixed(2)}MB) to Cloudflare R2`,
    );
  ...
  async addMultipleProductFiles(
    ...
    if (failedFiles.length === files.length) {
      throw new Error(
        `All files failed validation: ${failedFiles.map((f) => `${f.file.name}: ${f.error}`).join(", ")}`,
      );
    }
    ...
    const validFiles = files.filter(
      (file) => !failedFiles.find((f) => f.file === file),
    );
    ...
    for (
      ...
      const batchPromises = batch.map(async (file, batchIndex) => {
        ...
        const fileData: Partial<ProductFileInsert> = {
          ...
          is_primary:
            !hasPrimaryFile && actualIndex === 0 && uploadedFiles.length === 0,
          display_title:
            titles?.[actualIndex] ||
            file.name.replace(/\.[^/.]+$/, "").replace(/[_-]/g, " "),
        };
        ...
        try {
          ...
          const uploadedFile = await this.addProductFile(
            productId,
            file,
            fileData,
          );
  ...
  detectFileType(
    mimeType: string,
    fileName?: string,
  ): "video" | "document" | "audio" | "image" | "other" {
  ...
  getStorageFilePath(
    ...
    return STORAGE_CONFIG.getFilePath(
      expertId,
      productId,
      fileId,
  ...
  async createProductWithMultipleFiles(
    ...
    if (invalidFiles.length > 0) {
      const errors = invalidFiles
        .map((f) => `${f.file.name}: ${f.validation.error}`)
        .join(", ");
    ...
    const product = await this.createProduct({
      ...
      expert_id: productData.expert_id,
      content_type:
        productData.content_type || (files.length > 1 ? "bundle" : "single"),
    ...
    try {
      ...
      if (thumbnail) {
        try {
          ...
          console.warn(
            "Thumbnail upload failed, continuing with files:",
            error,
          );
        }
      }
      ...
      const primaryFile =
        uploadedFiles.find((f) => f.is_primary) || uploadedFiles[0];
      ...
      console.log(
        `Successfully created product ${product.id} with ${uploadedFiles.length} files`,
      );
      ...
      try {
        ...
        for (const uploadedFile of uploadedFiles) {
          try {
            ...
            console.warn(
              `Failed to cleanup file ${uploadedFile.id}:`,
              cleanupError,
            );
          }
        }
  ...
  async getExpertSalesAnalytics(expertId: string): Promise<{
    ...
    // Calculate totals
    const totalRevenue =
      sales?.reduce((sum, sale) => sum + (sale.amount || 0), 0) || 0;
