...
serve(async (req) => {
  ...
  try {
    ...
    if (product.file_url && product.file_url.trim() !== '' && product.file_url !== 'null') {
      ...
      if (product.file_url.startsWith('http')) {
        ...
        console.log('Generating signed URL for storage path:', product.file_url)
        
        // First, check if the file exists
        const { data: fileExists, error: checkError } = await supabaseClient.storage
          .from('products')
