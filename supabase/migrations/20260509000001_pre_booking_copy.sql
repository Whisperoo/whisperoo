-- Pre-book modal copy overrides (optional; defaults come from app i18n)
-- inquiry_prebook_message: optional full body shown before confirming inquiry booking
-- hospital_prebook_message: optional full body before confirming hospital-resource flow

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS inquiry_prebook_message text;

COMMENT ON COLUMN public.profiles.inquiry_prebook_message IS
  'Optional. When set, replaces default pre-confirm inquiry copy on expert profile. Otherwise app uses localized templates.';

ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS hospital_prebook_message text;

COMMENT ON COLUMN public.products.hospital_prebook_message IS
  'Optional. When booking_model=hospital and set, replaces default pre-confirm copy. How-to-schedule still shows after confirmation.';

-- Extend expert creation RPC
DROP FUNCTION IF EXISTS public.fn_admin_create_expert(
  text, text, text, text, text[], text[], integer, numeric, text, numeric, uuid, text, text
);
