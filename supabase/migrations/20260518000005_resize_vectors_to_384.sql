...
-- All tables are guarded with existence checks so this runs safely on any environment.

-- ── expert_documents (may not exist on all envs) ──────────────────────────────
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='expert_documents') THEN
    ...
    UPDATE public.expert_documents SET embedding = NULL WHERE embedding IS NOT NULL;
    ALTER TABLE public.expert_documents ALTER COLUMN embedding TYPE vector(384);
    CREATE INDEX expert_documents_embedding_idx ON public.expert_documents USING ivfflat (embedding vector_cosine_ops);
  END IF;
END;
$$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='compliance_training') THEN
    UPDATE public.compliance_training SET embedding = NULL WHERE embedding IS NOT NULL;
    ALTER TABLE public.compliance_training ALTER COLUMN embedding TYPE vector(384);
  END IF;
END;
$$;

-- ── profiles.expert_embedding ─────────────────────────────────────────────────
DO $$
BEGIN
  IF EXISTS (
...
-- ── expert_embeddings table ───────────────────────────────────────────────────
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='expert_embeddings') THEN
