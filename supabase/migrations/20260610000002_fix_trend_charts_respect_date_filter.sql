-- Fix fn_get_admin_dashboard: make enrollment, escalation, and checklist trend
-- charts respect the selected date range instead of always showing last 6 months.
--
-- Root cause: all three trend sub-queries used v_6m_ago hardcoded, ignoring
-- p_start_date / p_end_date entirely. Charts and KPI boxes therefore reflected
-- different time windows with the same date picker.
...
BEGIN
  ...
  -- 1. Total Enrolled (in selected date range)
  ...
  -- 9. Education Engagement Metrics
  ...
  -- Assemble & Return
  -- NOTE: All three trend charts now use v_effective_start / v_effective_end
  -- so they reflect the same date range as the KPI boxes.
  v_result := jsonb_build_object(
    ...
    'enrollment_trend',  COALESCE((
      ...
      FROM (
        ...
        WHERE p.onboarded = true
          AND (p_tenant_id IS NULL OR p.tenant_id = p_tenant_id)
          AND p.created_at >= v_effective_start
          AND p.created_at <= v_effective_end
    ...
    'escalation_trend',  COALESCE((
      ...
      FROM (
        ...
        WHERE msg.role = 'user'
          AND (p_tenant_id IS NULL OR p.tenant_id = p_tenant_id)
          AND msg.created_at >= v_effective_start
          AND msg.created_at <= v_effective_end
    ...
    'checklist_trend',   COALESCE((
      ...
      FROM (
        ...
        JOIN profiles p ON p.id = cp.user_id
        WHERE (p_tenant_id IS NULL OR p.tenant_id = p_tenant_id)
          AND cp.created_at >= v_effective_start
          AND cp.created_at <= v_effective_end
