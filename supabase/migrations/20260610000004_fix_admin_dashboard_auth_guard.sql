-- Fix fn_get_admin_dashboard and fn_get_resource_utilization: replace hardcoded
-- email allowlist with account_type check so all super_admin accounts can access
-- the dashboard (not just engineering@whisperoo.app).
...
BEGIN
  -- Guard: any admin or super_admin account
  SELECT account_type, lower(COALESCE(email, ''))
