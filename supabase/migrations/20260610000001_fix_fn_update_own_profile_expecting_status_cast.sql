-- Fix fn_update_own_profile: cast `expecting_status` column to its enum type, not text.
--
-- Root cause: the dynamic SQL builder fell through to the default `::text` cast
-- for the `expecting_status` column. PostgreSQL cannot implicitly coerce text to a
-- custom enum (expecting_status), so saving profile changes that include
-- `expecting_status` raised:
--   "column 'expecting_status' is of type expecting_status but expression is of type text"
...
BEGIN
  ...
  LOOP
    IF v_col = ANY(v_allowed_columns) THEN
      ...
      ELSIF v_col = 'expecting_status' THEN
        v_sql := v_sql || 'expecting_status';
      ELSE
        v_sql := v_sql || 'text';
      END IF;
      v_has_set := true;
