-- Fix fn_admin_qr_signup_metrics: only count fully onboarded users as "signups".
--
-- Root cause: attributed_signups, unattributed, and by_qr all counted any profile
-- row regardless of whether the user completed onboarding. A user who scanned a QR
-- code and started signup but never finished was being counted as a signup.
-- Now all three counts require onboarded = true.
