-- Migration: Server-side function to create expert profiles
-- Since profiles.id is a FK to auth.users, we can't just insert directly from the client.
-- This SECURITY DEFINER function creates a placeholder auth user + profile in one step.
...
BEGIN
  ...
  -- Now update it with expert fields.
  UPDATE profiles SET
...
GRANT EXECUTE ON FUNCTION public.fn_admin_create_expert(text, text, text, text, text[], text[], integer, numeric, text, numeric) TO authenticated;
